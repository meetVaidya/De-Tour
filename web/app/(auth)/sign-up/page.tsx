...
export default function SignUpPage() {
  ...
  const handleAuthenticationSuccess = async (
    ...
    try {
      console.log(
        `Handling success for user ${user.uid}. Checking profile existence.`,
      );
      ...
      if (profileExists) {
        ...
        console.log(
          `Profile does not exist. Redirecting to onboarding with type: ${selectedUserType}`,
        );

        // CRITICAL CHECK: Ensure userType is valid before redirecting
        if (!selectedUserType) {
          console.error(
            "Error: selectedUserType is missing before onboarding redirect!",
          );
          setError(
            "An internal error occurred (user type missing). Please try signing up again.",
          );
  ...
  const handleGoogleSignUp = async () => {
    ...
    if (!userType) {
      setError(
        "Please select an account type (User or Merchant) before signing in with Google.",
      );
    ...
    console.log(
      `Initiating Google Sign-In with selected type: ${currentSelectedType}`,
    );
